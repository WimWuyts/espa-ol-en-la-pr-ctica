# C6+ · Unidad 7 «¡Opina y cuídate!» — LEESMIJ (levering)

Paarse vervolgcursus (C6+), Unidad 7 — **de laatste unit (P3-capstone)**. Hiermee is **versie 1 van C6+ (U0–U7) compleet**.
**Thema:** gezondheid & advies, maatschappij en milieu — «pura vida».
**Kerngrammatica:** ① pretérito imperativo afirmativo (tú) — regelmatig (cuida, come, vive) + 8 onregelmatige (ten, ven, pon, haz, di, sal, sé, ve) — mét aangehechte pronomina (cuídate, hazlo, dímelo) · ② opinar y argumentar: creo que / pienso que / en mi opinión + **indicativo** + conectoren (porque, además, por eso, sin embargo). **P3-capstone:** de mini-mening-strand (U1–U6) wordt hier een volwaardig opiniestuk.
**Buiten scope (leerplan, hard):** géén imperativo negativo/usted (= subjuntivo), géén futuro simple / condicional / subjuntivo.
**Parada op La Ruta:** Costa Rica 🇨🇷 (San José), «pura vida» — de digitale kaart toont Costa Rica met ★ ¡Estás aquí! (**fin de la ruta**). Volledige route: España → México → Colombia → Argentina → Perú → Costa Rica.
...
- **Extra** — 8 bronnen in 5 groepen (ProfeDeELE · Arche-ELE), geordend zoals de unit zelf.
...
- Hub: elk tabblad aangeklikt (geen lege panelen), **0 JS-fouten**, 19 zelfcorrigerende widgets + 12 spellen; kaart-klik op **Costa Rica → ★ + «🍽️ En la mesa» + fin de la ruta** geverifieerd.
- PowerPoint: beide `.pptx` openen via python-pptx, zip-integriteit OK, 0 XML-fouten, **paars (0 groen-lek, 471/463 hits)**, 21/20 dia's, 102 klik-onthullingen, 32 hyperlinks, 4 vaardigheden.

## Regenereren
```
python3 01-cursussen/06-vervolg/U7/gen_c6plus_u7_print.py            # → C6plus_U7.html → PDF (Chromium)
python3 spaans-motor/make_c6plus_u7_games.py && node spaans-motor/build.mjs
python3 03-build/web/patch_maps_c6plus.py                            # bakt de kaart-fiche in (idempotent, U0–U7)
python3 03-build/web/gen_c6plus_u7_web.py                            # → 03-build/web/C6plus_U7_web.html
python3 03-build/pptx/gen_c6plus_u7_docente.py                      # → C6plus_U7_docente/alumno.pptx
```
