...
# -*- coding: utf-8 -*-
"""
gen_c4u4_ppt.py — Interactieve PowerPoint C4 · Unidad 4 «La familia»
================================================================================
Gedeelde builder (één bron) → TWEE decks:
  · C4_U4_docente.pptx  — docentversie: vrije navigatie; antwoorden verschijnen bij
    klik (fade) + volledige oplossing & didactiek in de spreker-notities.
  · C4_U4_alumno.ppsx   — leerlingversie: GEEN docentnotities, GEEN kiosk; gewone
...
Thema U4: La familia · presentar & describir a personas · mi/tu · ser+adjetivo
(concordancia -o/-a) · muy/un poco. Huisstijl: unitkleur rood #D64550 (C4).
Spaans-eerst + NL-steun. Twee kleurlagen: cursusrood (navigatie) + functionele
taalsemantiek. Bron: 03-build/web/gen_c4u4_kgt.py · gen_c4u4_pdf.py ·
gen_c4u4_practica.py · reservoir/U4_cocktail.md
"""
...
OUT_DOCENTE = os.path.join(HERE, "C4_U4_docente.pptx")
OUT_ALUMNO_PPTX = os.path.join(HERE, "C4_U4_alumno.pptx")
OUT_ALUMNO = os.path.join(HERE, "C4_U4_alumno.ppsx")
...
def footer(s, tab="U4 · LA FAMILIA", page=None):
...
# ============================================================================
# C4 · UNIDAD 4 «La familia» — slides (survival). Herbruikt de machinerie.
# ============================================================================
FTAB = "U4 · LA FAMILIA"
...
def s01_title():
    ...
    chip(s, Inches(0.6), Inches(0.5), "C4 · LA RUTA · EL DESPEGUE · PARADA 4", fill=WHITE, tcolor=G, size=12)
    text(s, Inches(0.55), Inches(1.15), Inches(12.3), Inches(1.1),
         [[("La familia", {"size": 46, "bold": True, "color": WHITE, "font": DISPLAY})]])
    text(s, Inches(0.6), Inches(2.25), Inches(11.8), Inches(0.7),
         [[("Esta es mi madre. ", {"size": 30, "bold": True, "color": WHITE, "font": DISPLAY}),
           ("Es muy elegante.", {"size": 17, "italic": True, "color": GT})]])
    text(s, Inches(0.6), Inches(3.2), Inches(11.5), Inches(1.1),
         [[("Je ", {"size": 16, "color": WHITE}), ("familie voorstellen", {"size": 16, "bold": True, "color": WHITE}),
           (" en mensen ", {"size": 16, "color": WHITE}), ("beschrijven", {"size": 16, "bold": True, "color": WHITE}),
           (" (het uiterlijk & het karakter).", {"size": 16, "color": WHITE})],
          [("Survival in Spanish — presentar y describir a las personas.", {"size": 13, "italic": True, "color": GT})]])
    # mochila-gids
    ...
    text(s, Inches(0.6), Inches(5.25), Inches(9), Inches(1.4),
         [[("En esta unidad vas a…", {"size": 13, "bold": True, "color": GD, "font": DISPLAY})],
          [("• Presentar a tu familia  ", {"size": 13, "color": INK}), ("esta es mi madre · se llama…", {"size": 11, "italic": True, "color": MUT})],
          [("• Describir a alguien  ", {"size": 13, "color": INK}), ("es alto/a · guapo/a · simpático/a", {"size": 11, "italic": True, "color": MUT})],
          [("• Graduar con muy / un poco  ", {"size": 13, "color": INK}), ("muy guapa · un poco tímido", {"size": 11, "italic": True, "color": MUT})]])
...
def s02_menu():
    ...
    tiles = [("1", "¡Escucha!", "la escena + chunks", 2),
             ("2", "Suena bien", "ll · y (yeísmo) · llana", 3),
             ("3", "Kit", "la familia · describir", 4),
             ("4", "Gramática", "mi/tu · ser + adjetivo", 6),
             ("5", "Práctica", "oefenen samen", 8),
             ("6", "Hablar", "presenta a un familiar", 9),
             ("7", "Música", "la familia hispana", 10),
             ("8", "Tarea", "Mi árbol de familia", 11),
    ...
    text(s, Inches(0.6), Inches(5.95), Inches(12), Inches(0.9),
         [[("Consejo · Tip. ", {"size": 12, "bold": True, "color": GD, "font": DISPLAY}),
           ("Luister eerst, spreek na, en durf zelf te beschrijven. Fouten maken hoort erbij.", {"size": 12, "italic": True, "color": INK})]])
    footer(s, tab=FTAB, page=pg())
...
VIDEO_ID="guCpaHfy7VQ"; VIDEO_TOP="Sitcom · Episodio 4"; VIDEO_MAIN="La familia"
VIDEO_POSTER=os.path.join(HERE,"assets","video_poster_U4.png")
...
def s03_escucha():
    s = slide(); bg(s)
    sectionbar(s, "§1 · ¡ESCUCHA!", "Bekijk la escena y escucha", "Kijk & luister — una madre toont el álbum de fotos", num=1)
    card(s, Inches(0.55), Inches(1.55), Inches(7.4), Inches(4.9), fill=WHITE, line=LINE)
    dia = [("Madre", "¿Es un chico? ¿Y es simpático?", F_SUBJ),
           ("Madre", "Esta es mi madre, la abuela de María.", F_SUBJ),
           ("Madre", "Es muy elegante, pero un poco gorda.", F_VERB),
           ("Julio", "María es una chica muy delgada y muy guapa.", F_SUBJ),
           ("Madre", "El tío Fermín es el guapo de la familia.", F_SUBJ),
           ("Madre", "Es muy alto y muy fuerte.", F_VERB),
           ("Madre", "Paula es la hermana de María.", F_SUBJ),
           ("Madre", "Vive en Londres. ¡Divertida, amable…!", F_PLAC)]
    y = Inches(1.8)
    ...
    text(s, Inches(8.45), Inches(1.72), Inches(4.1), Inches(2.2),
         [[("Chunks para llevar 🎒", {"size": 13, "bold": True, "color": GD, "font": DISPLAY})],
          [("la familia · mi madre / mi padre", {"size": 11.5, "color": INK})],
          [("esta es… · es la hermana de…", {"size": 11.5, "color": INK})],
          [("es alto · guapo · simpático", {"size": 11.5, "color": INK})],
          [("muy… · un poco…", {"size": 11.5, "color": INK})]])
    # echte, afspeelbare video (online YouTube-embed) — speelt in PowerPoint
    text(s, Inches(8.2), Inches(4.12), Inches(4.6), Inches(0.3),
         [[("🎬 Sitcom · Episodio 4 — klik om af te spelen", {"size": 11, "bold": True, "color": GD, "font": DISPLAY})]])
    add_online_video(s, VIDEO_ID, Inches(8.2), Inches(4.45), Inches(4.6), Inches(2.55), VIDEO_POSTER)
    footer(s, tab=FTAB, page=pg())

def s04_kit():
    s = slide(); bg(s)
    sectionbar(s, "§2 · KIT", "La familia y presentar", "De gezinsleden & iemand voorstellen", num=2)
    cols = [("La familia · miembros", [("la madre · el padre", "moeder · vader"), ("el hermano · la hermana", "broer · zus"),
              ("el abuelo · la abuela", "opa · oma"), ("el tío · la tía", "oom · tante"), ("el hijo · la hija", "zoon · dochter")]),
            ("Presentar a alguien", [("Esta es mi madre", "dit is mijn moeder"), ("Este es mi padre", "dit is mijn vader"),
              ("Es mi hermano/a", "mijn broer/zus"), ("Se llama…", "hij/zij heet…"), ("Es la hermana de María", "de zus van María")]),
            ("¿Dónde vive?", [("¿Dónde vives?", "waar woon je?"), ("Vivo en…", "ik woon in…"),
              ("Vive en Londres", "woont in Londen"), ("en la calle…", "in de straat…"), ("¿Y tú?", "en jij?")])]
...
def s05_presentarse():
    s = slide(); bg(s)
    sectionbar(s, "§2 · KIT", "Describir · el físico y el carácter", "Beschrijven: het uiterlijk & het karakter", num=2)
    left = [("es alto/a · bajo/a", "lang · klein"), ("es guapo/a", "knap"), ("es delgado/a · gordo/a", "slank · mollig"),
            ("es fuerte", "sterk"), ("tiene el pelo largo/corto", "lang/kort haar")]
    right = [("es simpático/a", "aardig"), ("es divertido/a", "grappig"), ("es amable", "vriendelijk"),
             ("es inteligente", "intelligent"), ("es elegante", "elegant")]
    card(s, Inches(0.55), Inches(1.6), Inches(6.0), Inches(4.9), fill=GT, line=G)
    text(s, Inches(0.8), Inches(1.78), Inches(5.5), Inches(0.5),
         [[("El físico · el aspecto", {"size": 13.5, "bold": True, "color": GD, "font": DISPLAY})]])
    y = Inches(2.5)
    for es, nl in left:
    ...
    text(s, Inches(7.05), Inches(1.78), Inches(5.5), Inches(0.5),
         [[("El carácter · la personalidad", {"size": 13.5, "bold": True, "color": GD, "font": DISPLAY})]])
    y = Inches(2.5)
    for es, nl in right:
    ...
    text(s, Inches(7.05), Inches(5.7), Inches(5.5), Inches(0.7),
         [[("¡Ojo! ", {"size": 12, "bold": True, "color": RED}),
           ("muy = heel · un poco = een beetje. Beschrijven = ser (es).", {"size": 12, "italic": True, "color": INK})]])
    footer(s, tab=FTAB, page=pg())

def s06_gram_ser():
    s = slide(); bg(s)
    sectionbar(s, "§4 · GRAMÁTICA", "mi · tu · su", "Van wie is het? De posesivos — functioneel", num=4)
    legend_func(s, Inches(0.55), Inches(1.42))
    # mi/tu/su
    card(s, Inches(0.55), Inches(1.9), Inches(6.0), Inches(2.2), fill=WHITE, line=LINE)
    text(s, Inches(0.8), Inches(2.05), Inches(5.5), Inches(0.4), [[("mi · tu · su — van wie is het familielid?", {"size": 15, "bold": True, "color": GD, "font": DISPLAY})]])
    est = [("mi", "mijn", "mi madre · mi hermano"), ("tu", "jouw", "¿Y tu familia?"), ("su", "zijn/haar", "su padre (de él/ella)")]
    y = Inches(2.55)
    for p, nl, ex in est:
        text(s, Inches(0.8), y, Inches(5.6), Inches(0.4),
             [[(p + "  ", {"size": 13, "bold": True, "color": F_SUBJ}), (nl, {"size": 13, "color": INK}), ("   " + ex, {"size": 11, "italic": True, "color": MUT})]])
        y = y + Inches(0.48)
    # ejemplos + ¡Ojo!
    card(s, Inches(6.8), Inches(1.9), Inches(6.0), Inches(2.2), fill=WHITE, line=LINE)
    text(s, Inches(7.05), Inches(2.05), Inches(5.5), Inches(0.4), [[("de + naam · ejemplos", {"size": 15, "bold": True, "color": GD, "font": DISPLAY})]])
    ej = [("Esta es ", "mi madre.", F_SUBJ), ("la hermana ", "de María", F_SUBJ), ("¿Y ", "tu familia?", F_SUBJ)]
    y = Inches(2.55)
    for head, tail, col in ej:
        text(s, Inches(7.05), y, Inches(5.6), Inches(0.4),
             [[(head, {"size": 13, "color": INK}), (tail, {"size": 13, "bold": True, "color": col})]])
        y = y + Inches(0.42)
    text(s, Inches(7.05), Inches(3.55), Inches(5.6), Inches(0.5),
         [[("¡Ojo! ", {"size": 11.5, "bold": True, "color": RED}), ("mi/tu blijven gelijk: mi madre én mi padre.", {"size": 11.5, "italic": True, "color": INK})]])
    ...
    text(s, Inches(0.8), Inches(4.5), Inches(11.7), Inches(0.5),
         [[("Completa · vul aan (klik voor de oplossing): ", {"size": 13, "bold": True, "color": GD, "font": DISPLAY}),
           ("Esta es ___ madre (van mij).  ¿Y ___ familia? (van jou)  ___ padre (van hem). ", {"size": 13, "color": INK})]])
    exercise_solucion(s, Inches(0.8), Inches(5.15), Inches(11.7), Inches(0.7),
        [[("mi · tu · su", {"bold": True, "color": GD, "size": 13})]])
    noodroute(s)
    notes(s, "Functioneel: enkel de vaste vormen mi / tu / su (herkennen & gebruiken). Géén volledig possessief-systeem (nuestro, vuestro, plural) — dat is C5. Kern: mi/tu blijven gelijk (geen -o/-a): mi madre én mi padre. Alternatief: de + naam (la hermana de María).")
    footer(s, tab=FTAB, page=pg())

def s07_gram_mv():
    s = slide(); bg(s)
    sectionbar(s, "§4 · GRAMÁTICA", "es + adjetivo ♂/♀", "Iemand beschrijven: het adjectief past zich aan", num=4)
    ...
    text(s, Inches(0.8), Inches(1.9), Inches(5.5), Inches(2.0),
         [[("♂ Un chico es…", {"size": 14, "bold": True, "color": RGBColor(0x1E,0x40,0xAF), "font": DISPLAY})],
          [("alto · guapo · delgado", {"size": 18, "bold": True, "color": RGBColor(0x1E,0x40,0xAF)})],
          [("simpático · divertido", {"size": 14, "bold": True, "color": RGBColor(0x1E,0x40,0xAF)})]])
    ...
    text(s, Inches(7.05), Inches(1.9), Inches(5.5), Inches(2.0),
         [[("♀ Una chica es…", {"size": 14, "bold": True, "color": RGBColor(0x9D,0x17,0x4D), "font": DISPLAY})],
          [("alta · guapa · delgada", {"size": 18, "bold": True, "color": RGBColor(0x9D,0x17,0x4D)})],
          [("simpática · divertida", {"size": 14, "bold": True, "color": RGBColor(0x9D,0x17,0x4D)})]])
    ...
    text(s, Inches(0.8), Inches(4.4), Inches(11.7), Inches(1.5),
         [[("Op -e blijft gelijk (♂ = ♀)  ·  muy / un poco", {"size": 15, "bold": True, "color": GD, "font": DISPLAY})],
          [("Eindigt op -e → gelijk voor ♂ én ♀: ", {"size": 13, "color": INK}), ("amable · inteligente · elegante · fuerte", {"size": 13, "bold": True, "color": F_SUBJ}), (".", {"size": 13, "color": INK})],
          [("¡Ojo! ", {"size": 13, "bold": True, "color": RED}), ("Beschrijven = ", {"size": 13, "color": INK}), ("ser", {"size": 13, "bold": True, "color": F_VERB}),
           (" (es guapa), niet estar. ", {"size": 13, "color": INK}), ("muy", {"size": 13, "bold": True, "color": INK}), (" = heel · ", {"size": 13, "color": INK}), ("un poco", {"size": 13, "bold": True, "color": INK}), (" = een beetje (muy alta · un poco gorda).", {"size": 13, "color": INK})]])
    footer(s, tab=FTAB, page=pg())

def s08_practica():
    s = slide(); bg(s)
    sectionbar(s, "§3 · PRÁCTICA", "Completa la descripción", "Vul samen aan — klik voor de oplossing", num=3)
    card(s, Inches(0.55), Inches(1.7), Inches(7.6), Inches(4.5), fill=WHITE, line=LINE)
    lines = [("— ¿Quién es esta chica?", ""),
             ("— ___ mi hermana. Es muy simpátic__.", "Es · simpática (♀)"),
             ("— ¿Y este chico?", ""),
             ("— Es ___ hermano. Es alt__ y un poco tímido.", "mi · alto (♂)")]
    y = Inches(2.0)
    for q, _a in lines:
    ...
    exercise_solucion(s, Inches(8.4), Inches(1.9), Inches(4.4), Inches(4.0),
        [[("1. Es", {"color": GD, "size": 14})], [("2. simpática (♀)", {"color": GD, "size": 14})],
         [("3. mi", {"color": GD, "size": 14})], [("4. alto (♂)", {"color": GD, "size": 14})]],
        title_doc="SOLUCIÓN · docent")
    noodroute(s)
    notes(s, "Laat leerlingen eerst zelf proberen (in duo). Klik daarna de oplossing open. Let op: ser (es) om te beschrijven, de concordancia ♂/♀ (simpático/simpática, alto/alta) en mi/tu (mi hermana, mi hermano).")
    footer(s, tab=FTAB, page=pg())

def s09_speaking():
    s = slide(); bg(s)
    sectionbar(s, "§3 · HABLAR", "Presenta a un familiar", "Stel een familielid voor & beschrijf hem/haar — sin leer", num=3)
    ...
    text(s, Inches(0.85), Inches(1.95), Inches(7.1), Inches(3.0),
         [[("Modelo · zeg dit hardop:", {"size": 13, "bold": True, "color": GD, "font": DISPLAY})],
          [("«— Esta es mi madre.", {"size": 18, "color": INK})],
          [("— Se llama Ana. Es muy simpática.", {"size": 18, "color": INK})],
          [("— Es alta y un poco tímida.", {"size": 18, "color": INK})],
          [("— Vive en Sevilla.»", {"size": 16, "color": INK})]])
    ...
    text(s, Inches(8.65), Inches(1.95), Inches(3.9), Inches(3.0),
         [[("¿Cómo? · Werkvorm", {"size": 13, "bold": True, "color": GD, "font": DISPLAY})],
          [("1. Kies een familielid (echt of fantasie).", {"size": 13, "color": INK})],
          [("2. Presenta: «Esta/Este es mi… Se llama…»", {"size": 13, "color": INK})],
          [("3. Describe: 2 adjectieven (♂/♀ + muy/un poco).", {"size": 13, "color": INK})],
...
def s10_musica():
    s = slide(); bg(s)
    sectionbar(s, "CULTURA", "La familia en el mundo hispano", "Grote, warme families + muziek die jullie kennen", num=None)
    ...
    text(s, Inches(0.85), Inches(5.65), Inches(11.7), Inches(0.8),
         [[("👨‍👩‍👧‍👦 La familia es grande y cercana ", {"size": 13, "bold": True, "color": GD, "font": DISPLAY}),
           ("(la sobremesa = lang napraten aan tafel · dos apellidos · Frida Kahlo schilderde haar stamboom). La playlist + LyricsTraining op de digitale hub → tabblad Música.", {"size": 12, "color": INK})]])
    footer(s, tab=FTAB, page=pg())

def s11_tarea():
    s = slide(); bg(s)
    sectionbar(s, "§5 · TAREA FINAL", "Mi árbol de familia", "Stel 4 familieleden voor & beschrijf ze", num=5)
    ...
    text(s, Inches(0.75), Inches(1.78), Inches(5.6), Inches(0.4), [[("MI ÁRBOL · noteer je frames", {"size": 12, "bold": True, "color": WHITE, "font": DISPLAY})]])
    text(s, Inches(0.85), Inches(2.5), Inches(5.4), Inches(2.5),
         [[("👩 Esta es mi ", {"size": 14, "bold": True, "color": INK}), ("____. Se llama ____. Es ____.", {"size": 14, "color": LINE})],
          [("👨 Este es mi ", {"size": 14, "bold": True, "color": INK}), ("____. Es ____ y ____.", {"size": 14, "color": LINE})],
          [("🧑 Est_ es mi ", {"size": 14, "bold": True, "color": INK}), ("____. Es muy ____.", {"size": 14, "color": LINE})]])
    ...
    text(s, Inches(7.05), Inches(1.9), Inches(5.5), Inches(3.1),
         [[("Los pasos · stappen", {"size": 14, "bold": True, "color": GD, "font": DISPLAY})],
          [("1. Dibuja el árbol: 4 personen (madre, padre…).", {"size": 13, "color": INK})],
          [("2. Presenta a cada uno: «Est_ es mi… Se llama…»", {"size": 13, "color": INK})],
          [("3. Describe: 2 adjectieven (♂/♀ + muy/un poco).", {"size": 13, "color": INK})],
    ...
    text(s, Inches(0.6), Inches(5.5), Inches(12), Inches(0.9),
         [[("🏁 Klaar als… ", {"size": 13, "bold": True, "color": GD, "font": DISPLAY}),
           ("je 4 familieleden voorstelt met «est_ es mi…», elk met 2 adjectieven (juiste ♂/♀ + muy/un poco), en de stamboom aanwijst — zónder af te lezen.", {"size": 12.5, "color": INK})]])
    footer(s, tab=FTAB, page=pg())
...
def s12_repaso():
    ...
    text(s, Inches(0.85), Inches(1.9), Inches(7.1), Inches(4.0),
         [[("Zo stel je iemand voor", {"size": 14, "bold": True, "color": GD, "font": DISPLAY})],
          [("Esta/Este es mi ___ (madre/padre…). Se llama ___.", {"size": 14, "color": INK})],
          [("mi = van mij · tu = van jou · de + naam", {"size": 13, "color": INK})],
          [("", {"size": 6})],
          [("Zo beschrijf je iemand", {"size": 14, "bold": True, "color": GD, "font": DISPLAY})],
          [("Es alto/alta · guapo/guapa · simpático/a", {"size": 13, "color": INK})],
          [("-o = ♂ · -a = ♀  |  -e blijft gelijk (amable)", {"size": 13, "color": INK})],
          [("muy = heel · un poco = een beetje · ser (es)", {"size": 13, "color": INK})]])
    ...
    items = ["mijn familie voorstellen (est_ es mi…)", "iemand beschrijven (ser + adjetivo, ♂/♀)", "mi/tu en muy/un poco juist gebruiken", "de familieleden benoemen"]
    y = Inches(2.55)
    for it in items:
        text(s, Inches(8.65), y, Inches(3.9), Inches(0.7), [[("🟢🟡🔴  ", {"size": 12}), (it, {"size": 12, "color": INK})]])
        y = y + Inches(0.72)
    text(s, Inches(8.65), Inches(5.5), Inches(3.9), Inches(0.7), [[("🎮 Repasa jugando", {"size": 12, "bold": True, "color": GD, "font": DISPLAY})], [("online op de hub · árbol de familia", {"size": 11, "italic": True, "color": MUT})]])
    footer(s, tab=FTAB, page=pg())

def s13_teacher():
    s = slide(); bg(s, color=RGBColor(0x24,0x1C,0x1B))
    text(s, Inches(0.6), Inches(0.5), Inches(12), Inches(0.7), [[("Docentendossier · Unidad 4 «La familia»", {"size": 22, "bold": True, "color": WHITE, "font": DISPLAY})]])
    text(s, Inches(0.6), Inches(1.4), Inches(12.1), Inches(5.4),
         [[("Timing (2 lesuren van 50 min).", {"size": 14, "bold": True, "color": RGBColor(0xFB,0xEA,0xEC), "font": DISPLAY})],
          [("Les 1: Escucha (sitcom ep. 4 · id guCpaHfy7VQ, el álbum de fotos) + Suena bien (ll · y/yeísmo · acento llano) + Kit (la familia · presentar · describir). Les 2: gramática functioneel (mi/tu · ser+adjetivo ♂/♀ · muy/un poco), hablar «presenta a un familiar», tarea «Mi árbol de familia» + música.", {"size": 12.5, "color": RGBColor(0xEC,0xEA,0xE3)})],
          [("", {"size": 6})],
          [("Aanpak C4 (survival).", {"size": 14, "bold": True, "color": RGBColor(0xFB,0xEA,0xEC), "font": DISPLAY})],
          [("Chunks komen auditief binnen (luisteren → naspreken). mi/tu en ser+adjetivo enkel functioneel (vaste vormen, herkennen & gebruiken), géén volledig possessief-/adjectiefsysteem — dat is C5. Recycle uit U3: concordancia -o/-a (nu op adjetivos). Kernvalstrik: beschrijven = ser (es), niet estar; -e blijft gelijk (amable). Doelcodes: C4-SP-1/2 · C4-WS-1 · C4-MEC-1/2 · C4-STR-1 · C4-CU-1.", {"size": 12.5, "color": RGBColor(0xEC,0xEA,0xE3)})],
          [("", {"size": 6})],
          [("Evaluatie.", {"size": 14, "bold": True, "color": RGBColor(0xFB,0xEA,0xEC), "font": DISPLAY})],
          [("Mondelinge mini-taak («Mi árbol de familia»: 4 personen voorstellen + beschrijven met país nvt, ser+adjetivo ♂/♀ + muy/un poco) + herkennen miembro/adjetivo. Geen leerplan → focus op «kunnen gebruiken in de praktijk».", {"size": 12.5, "color": RGBColor(0xEC,0xEA,0xE3)})],
          [("", {"size": 6})],
...
def s_uitspraak():
    s = slide(); bg(s)
    sectionbar(s, "SUENA BIEN", "La ll · la y (yeísmo) · el acento llano", "De ll, de y & de klemtoon — luister en spreek na", num=None)
    # ll-kaart
    ...
    text(s,Inches(0.8),Inches(1.85),Inches(5.6),Inches(0.4),[[("La elle · ll — de klank van «calle»",{"size":14,"bold":True,"color":GD,"font":DISPLAY})]])
    text(s,Inches(0.8),Inches(2.35),Inches(5.6),Inches(1.3),
         [[("calle · llave · apellido · ella · silla · llamar",{"size":15,"bold":True,"color":INK})],
          [("Klinkt in de meeste landen als de NL «j» in «ja»: ca-je.",{"size":11,"italic":True,"color":MUT})]])
    # y/yeísmo-kaart
    ...
    text(s,Inches(7.0),Inches(1.85),Inches(5.6),Inches(0.4),[[("La i griega · y — el yeísmo",{"size":14,"bold":True,"color":GD,"font":DISPLAY})]])
    text(s,Inches(7.0),Inches(2.35),Inches(5.6),Inches(1.3),
         [[("yo · ya · playa · mayo · desayuno · leyenda",{"size":15,"bold":True,"color":INK})],
          [("De y klinkt nét als de ll (dat heet yeísmo): «yo» ≈ «jo».",{"size":11,"italic":True,"color":MUT})]])
    ...
    text(s,Inches(0.85),Inches(4.05),Inches(11.7),Inches(0.6),
         [[("¡Ojo! ",{"size":13,"bold":True,"color":RED,"font":DISPLAY}),("«ll» is één letter/klank, niet twee l's: ca",{"size":13,"color":INK}),("ll",{"size":13,"bold":True,"color":GD}),("e, ",{"size":13,"color":INK}),("ll",{"size":13,"bold":True,"color":GD}),("ave, ape",{"size":13,"color":INK}),("ll",{"size":13,"bold":True,"color":GD}),("ido. En de y klinkt nét zo (yeísmo).",{"size":13,"color":INK})]])
    text(s,Inches(0.6),Inches(5.0),Inches(12),Inches(0.4),[[("El acento llano · de meeste woorden — klemtoon op de voorlaatste lettergreep",{"size":14,"bold":True,"color":GD,"font":DISPLAY})]])
    text(s,Inches(0.6),Inches(5.55),Inches(12.2),Inches(0.7),
         [[("MA",{"color":G,"bold":True}),("·dre      her·",{}),("MA",{"color":G,"bold":True}),("·no      a·",{}),("BUE",{"color":G,"bold":True}),("·la      e·le·",{}),("GAN",{"color":G,"bold":True}),("·te",{})]],size=20,font=DISPLAY)
