...
# -*- coding: utf-8 -*-
"""
gen_c4u6_ppt.py — Interactieve PowerPoint C4 · Unidad 6 «La casa y los lugares»
================================================================================
Gedeelde builder (één bron) → TWEE decks:
  · C4_U6_docente.pptx  — docentversie: vrije navigatie; antwoorden verschijnen bij
    klik (fade) + volledige oplossing & didactiek in de spreker-notities.
  · C4_U6_alumno.ppsx   — leerlingversie: GEEN docentnotities, GEEN kiosk; gewone
...
Thema U6: La casa y los lugares · las habitaciones · preposiciones de lugar
(encima de · debajo de · dentro de… · de+el=del) · hay vs. está · poder (¿puedo? /
¿puedes? — pedir permiso). Huisstijl: unitkleur rood #D64550 (C4). Spaans-eerst +
NL-steun. Twee kleurlagen: cursusrood (navigatie) + functionele taalsemantiek
(plaats/preposities = turquoise). Bron: 03-build/web/gen_c4u6_kgt.py ·
gen_c4u6_pdf.py · gen_c4u6_practica.py · gen_c4u6_escucha.py
"""
...
OUT_DOCENTE = os.path.join(HERE, "C4_U6_docente.pptx")
OUT_ALUMNO_PPTX = os.path.join(HERE, "C4_U6_alumno.pptx")
OUT_ALUMNO = os.path.join(HERE, "C4_U6_alumno.ppsx")
...
F_PLAC = RGBColor(0x0E, 0x9E, 0x97)  # plaats/preposities  (turquoise · U6)
...
def footer(s, tab="U6 · LA CASA", page=None):
...
# ============================================================================
# C4 · UNIDAD 6 «La casa y los lugares» — slides (survival). Herbruikt de machinerie.
# ============================================================================
FTAB = "U6 · LA CASA"
...
def s01_title():
    ...
    chip(s, Inches(0.6), Inches(0.5), "C4 · LA RUTA · EL DESPEGUE · PARADA 6", fill=WHITE, tcolor=G, size=12)
    text(s, Inches(0.55), Inches(1.15), Inches(12.3), Inches(1.1),
         [[("La casa y los lugares", {"size": 46, "bold": True, "color": WHITE, "font": DISPLAY})]])
    text(s, Inches(0.6), Inches(2.25), Inches(11.8), Inches(0.7),
         [[("¿Dónde está el bolso? ", {"size": 26, "bold": True, "color": WHITE, "font": DISPLAY}),
           ("— Está encima de la mesa.", {"size": 17, "italic": True, "color": GT})]])
    text(s, Inches(0.6), Inches(3.2), Inches(11.5), Inches(1.1),
         [[("De ", {"size": 16, "color": WHITE}), ("kamers", {"size": 16, "bold": True, "color": WHITE}),
           (" van je huis benoemen en zeggen ", {"size": 16, "color": WHITE}), ("waar alles staat", {"size": 16, "bold": True, "color": WHITE}),
           (" (preposiciones · hay/está · ¿puedo?).", {"size": 16, "color": WHITE})],
          [("Survival in Spanish — describir la casa y situar las cosas.", {"size": 13, "italic": True, "color": GT})]])
    # mochila-gids
    ...
    text(s, Inches(0.6), Inches(5.25), Inches(9), Inches(1.4),
         [[("En esta unidad vas a…", {"size": 13, "bold": True, "color": GD, "font": DISPLAY})],
          [("• Nombrar las habitaciones  ", {"size": 13, "color": INK}), ("la cocina · el salón · el dormitorio…", {"size": 11, "italic": True, "color": MUT})],
          [("• Usar preposiciones de lugar  ", {"size": 13, "color": INK}), ("encima de · debajo de · al lado del…", {"size": 11, "italic": True, "color": MUT})],
          [("• Decir dónde está y pedir permiso  ", {"size": 13, "color": INK}), ("hay/está · ¿puedo…? / ¿puedes…?", {"size": 11, "italic": True, "color": MUT})]])
...
def s02_menu():
    ...
    tiles = [("1", "¡Escucha!", "la escena + chunks", 2),
             ("2", "Suena bien", "b = v · llana/aguda", 3),
             ("3", "Kit", "la casa · habitaciones", 4),
             ("4", "Gramática", "preposiciones · hay/está · poder", 6),
             ("5", "Práctica", "oefenen samen", 8),
             ("6", "Hablar", "describe una habitación", 9),
             ("7", "Música", "casas del mundo hispano", 10),
             ("8", "Tarea", "Plano de mi casa", 11),
    ...
    text(s, Inches(0.6), Inches(5.95), Inches(12), Inches(0.9),
         [[("Consejo · Tip. ", {"size": 12, "bold": True, "color": GD, "font": DISPLAY}),
           ("Teken je huis, benoem de kamers en zeg waar alles staat. Durf raden — fouten maken hoort erbij.", {"size": 12, "italic": True, "color": INK})]])
    footer(s, tab=FTAB, page=pg())
...
VIDEO_ID="oDu5s6q4YKg"; VIDEO_TOP="Sitcom · Episodio 6"; VIDEO_MAIN="La casa"
VIDEO_POSTER=os.path.join(HERE,"assets","video_poster_U6.png")
...
def s03_escucha():
    s = slide(); bg(s)
    sectionbar(s, "§1 · ¡ESCUCHA!", "Bekijk la escena y escucha", "Kijk & luister — Julio is nerveus: María's spullen liggen overal", num=1)
    card(s, Inches(0.55), Inches(1.55), Inches(7.4), Inches(4.9), fill=WHITE, line=LINE)
    dia = [("Julio", "Necesito café urgente, tengo mucho sueño. Estoy nervioso.", F_SUBJ),
           ("Julio", "María deja su crema en el cuarto de baño, se olvida el bolso en la cocina.", F_PLAC),
           ("Julio", "Hay cosas encima de todas las sillas, debajo de la cama.", F_PLAC),
           ("Julio", "Dentro del frigorífico tengo tres botellas de leche.", F_PLAC),
           ("Julio", "Ella quiere una casa, un nido.", F_OBJ),
           ("Julio", "¿Puedo fumar?", F_VERB),
           ("Fernando", "Aquí dentro no, pero puedes ir fuera.", F_VERB),
           ("Fernando", "No sé qué hace con Paul. Un empresario inglés con mucho éxito.", F_SUBJ)]
    y = Inches(1.8)
    ...
    text(s, Inches(8.45), Inches(1.72), Inches(4.1), Inches(2.2),
         [[("Chunks para llevar 🎒", {"size": 13, "bold": True, "color": GD, "font": DISPLAY})],
          [("encima de · debajo de · dentro del", {"size": 11.5, "color": INK})],
          [("la cocina · el cuarto de baño · la cama", {"size": 11.5, "color": INK})],
          [("¿dónde está…? · está en… · hay…", {"size": 11.5, "color": INK})],
          [("¿puedo…? · puedes ir fuera · aquí no", {"size": 11.5, "color": INK})]])
    # echte, afspeelbare video (online YouTube-embed) — speelt in PowerPoint
    text(s, Inches(8.2), Inches(4.12), Inches(4.6), Inches(0.3),
         [[("🎬 Sitcom · Episodio 6 — klik om af te spelen", {"size": 11, "bold": True, "color": GD, "font": DISPLAY})]])
    add_online_video(s, VIDEO_ID, Inches(8.2), Inches(4.45), Inches(4.6), Inches(2.55), VIDEO_POSTER)
    footer(s, tab=FTAB, page=pg())

def s04_kit():
    s = slide(); bg(s)
    sectionbar(s, "§2 · KIT", "La casa · habitaciones y muebles", "De kamers, de meubels & de plaats-woorden", num=2)
    cols = [("Las habitaciones", [("la cocina", "de keuken"), ("el salón", "de woonkamer"),
              ("el dormitorio", "de slaapkamer"), ("el cuarto de baño", "de badkamer"), ("la entrada · el pasillo", "hal · gang")]),
            ("En casa · muebles y cosas", [("la cama", "het bed"), ("el sofá · la mesa", "bank · tafel"),
              ("el armario", "de kast"), ("el frigorífico", "de koelkast"), ("la ventana · la puerta", "raam · deur")]),
            ("¿Dónde está? · preposiciones", [("encima de · debajo de", "op/boven · onder"), ("dentro de · entre", "in/binnen · tussen"),
              ("al lado de", "naast"), ("delante de · detrás de", "voor · achter"), ("aquí · ahí · fuera", "hier · daar · buiten")])]
...
def s05_presentarse():
    s = slide(); bg(s)
    sectionbar(s, "§2 · KIT", "Situar · ¿dónde está? & pedir permiso", "Zeggen waar iets is & om toestemming vragen", num=2)
    left = [("¿Dónde está…?", "waar is…?"), ("Está en…", "het is in/op…"), ("Hay … en la cocina", "er is … in de keuken"),
            ("aquí · ahí · fuera", "hier · daar · buiten"), ("está encima de…", "het staat op…")]
    right = [("¿Puedo…?", "mag/kan ik…?"), ("¿Puedes…?", "kan/mag jij…?"), ("Sí, puedes…", "ja, je mag…"),
             ("Aquí no", "hier niet"), ("puedes ir fuera", "je mag naar buiten")]
    card(s, Inches(0.55), Inches(1.6), Inches(6.0), Inches(4.9), fill=GT, line=G)
    text(s, Inches(0.8), Inches(1.78), Inches(5.5), Inches(0.5),
         [[("Situar · ¿dónde está?", {"size": 13.5, "bold": True, "color": GD, "font": DISPLAY})]])
    y = Inches(2.5)
    for es, nl in left:
    ...
    text(s, Inches(7.05), Inches(1.78), Inches(5.5), Inches(0.5),
         [[("Pedir permiso · poder", {"size": 13.5, "bold": True, "color": GD, "font": DISPLAY})]])
    y = Inches(2.5)
    for es, nl in right:
    ...
    text(s, Inches(7.05), Inches(5.7), Inches(5.5), Inches(0.7),
         [[("¡Ojo! ", {"size": 12, "bold": True, "color": RED}),
           ("de + el = del (encima del sofá) · poder + hele werkwoord (fumar, no fumo).", {"size": 12, "italic": True, "color": INK})]])
    footer(s, tab=FTAB, page=pg())

def s06_gram_ser():
    s = slide(); bg(s)
    sectionbar(s, "§4 · GRAMÁTICA", "Las preposiciones de lugar", "Plaats-woorden: zeggen waar iets is (turquoise)", num=4)
    legend_func(s, Inches(0.55), Inches(1.42))
    # preposiciones-kaart (links, breed)
    card(s, Inches(0.55), Inches(1.9), Inches(7.6), Inches(4.1), fill=WHITE, line=LINE)
    text(s, Inches(0.8), Inches(2.05), Inches(7.1), Inches(0.4), [[("¿Dónde está? — de plaats-woorden", {"size": 15, "bold": True, "color": GD, "font": DISPLAY})]])
    preps = [("encima de", "op / boven"), ("debajo de", "onder"), ("dentro de", "in / binnen"),
             ("al lado de", "naast"), ("delante de", "voor"), ("detrás de", "achter"), ("entre", "tussen")]
    y = Inches(2.55)
    for i, (p, nl) in enumerate(preps):
        col = i % 2; x = Inches(0.8) + col * Inches(3.7)
        if col == 0 and i > 0:
            y = y + Inches(0.48)
        text(s, x, y, Inches(3.6), Inches(0.4),
             [[(p + "  ", {"size": 14, "bold": True, "color": F_PLAC}), (nl, {"size": 11.5, "italic": True, "color": MUT})]])
    # del / al-kaart (rechts)
    card(s, Inches(8.4), Inches(1.9), Inches(4.4), Inches(4.1), fill=GT, line=G)
    text(s, Inches(8.65), Inches(2.05), Inches(3.9), Inches(0.4), [[("de + el = del · a + el = al", {"size": 15, "bold": True, "color": GD, "font": DISPLAY})]])
    text(s, Inches(8.65), Inches(2.7), Inches(3.9), Inches(2.8),
         [[("encima ", {"size": 14, "color": INK}), ("de + el", {"size": 14, "bold": True, "color": F_PLAC}), (" sofá", {"size": 14, "color": INK})],
          [("→ encima ", {"size": 15, "color": INK}), ("del", {"size": 15, "bold": True, "color": F_PLAC}), (" sofá", {"size": 15, "color": INK})],
          [("", {"size": 6})],
          [("al lado ", {"size": 14, "color": INK}), ("a + el", {"size": 14, "bold": True, "color": F_PLAC}), (" armario", {"size": 14, "color": INK})],
          [("→ al lado ", {"size": 15, "color": INK}), ("del", {"size": 15, "bold": True, "color": F_PLAC}), (" armario", {"size": 15, "color": INK})]])
    text(s, Inches(8.65), Inches(5.15), Inches(3.9), Inches(0.7),
         [[("¡Ojo! ", {"size": 11.5, "bold": True, "color": RED}), ("nooit «de el» of «a el».", {"size": 11.5, "italic": True, "color": INK})]])
    notes(s, "Functioneel: de preposiciones de lugar herkennen & gebruiken om te situeren (encima de · debajo de · dentro de · al lado de · delante de · detrás de · entre). Kernvalstrik: de+el=del en a+el=al (encima del sofá, al lado del armario — nooit «de el»). Kleurcode turquoise = plaats/preposities. Doelcodes: C4-TS-3 · C4-WS-1.")
    footer(s, tab=FTAB, page=pg())

def s07_gram_mv():
    s = slide(); bg(s)
    sectionbar(s, "§4 · GRAMÁTICA", "hay vs. está · poder (¿puedo?)", "Iets nieuw → hay · waar dat ding staat → está · toestemming → poder", num=4)
    # hay vs está-kaart
    card(s, Inches(0.55), Inches(1.65), Inches(7.6), Inches(2.5), fill=WHITE, line=LINE)
    text(s, Inches(0.8), Inches(1.8), Inches(7.1), Inches(0.4), [[("hay vs. está — er is / het staat", {"size": 15, "bold": True, "color": GD, "font": DISPLAY})]])
    filas = [("hay", "iets nieuw (bestaat er)", "En el salón hay un sofá."),
             ("¿dónde está…?", "waar is dat bekende ding?", "¿Dónde está el bolso?"),
             ("está / están", "het staat / ze staan", "Está encima de la mesa.")]
    y = Inches(2.35)
    ...
    text(s, Inches(8.65), Inches(1.8), Inches(3.9), Inches(0.4), [[("poder · pedir permiso", {"size": 15, "bold": True, "color": GD, "font": DISPLAY})]])
    text(s, Inches(8.65), Inches(2.35), Inches(3.9), Inches(1.7),
         [[("¿", {"size": 14, "color": INK}), ("Puedo", {"size": 14, "bold": True, "color": F_VERB}), ("…? ", {"size": 14, "color": INK}), ("ik", {"size": 11, "italic": True, "color": MUT})],
          [("¿", {"size": 14, "color": INK}), ("Puedes", {"size": 14, "bold": True, "color": F_VERB}), ("…? ", {"size": 14, "color": INK}), ("jij", {"size": 11, "italic": True, "color": MUT})],
          [("+ hele werkwoord", {"size": 12.5, "color": INK}), (" (fumar…)", {"size": 11, "italic": True, "color": MUT})],
          [("Sí, puedes · Aquí no", {"size": 12.5, "italic": True, "color": MUT})]])
    ...
    text(s, Inches(0.8), Inches(4.5), Inches(11.7), Inches(0.5),
         [[("Completa · vul aan (klik voor de oplossing): ", {"size": 13, "bold": True, "color": GD, "font": DISPLAY}),
           ("¿Dónde ___ el bolso? — ___ encima del sofá.  ¿___ mirar? Sí.", {"size": 13, "color": INK})]])
    exercise_solucion(s, Inches(0.8), Inches(5.15), Inches(11.7), Inches(0.7),
        [[("está · Está · Puedo", {"bold": True, "color": GD, "size": 13})]])
    noodroute(s)
    notes(s, "Functioneel: hay = iets bestaat/is er (nieuw, onbekend) → «hay un sofá». está = waar dat bekende ding zich bevindt → «¿dónde está el bolso? · está encima de la mesa». poder om permiso te vragen: ¿puedo…? (ik) / ¿puedes…? (jij) + hele werkwoord (fumar, venir). Géén volledig werkwoordsysteem — enkel deze vaste vormen (dat blijft C5). Oplossing: está · Está · Puedo.")
    footer(s, tab=FTAB, page=pg())
...
def s08_practica():
    ...
    lines = [("— ¿Dónde ___ mi bolso?", "está"),
             ("— ___ encima del sofá.", "Está"),
             ("— ¿Y las llaves? — Están ___ de la mesa.", "debajo"),
             ("— ¿Puedo mirar en la cocina? — Sí, ___.", "puedes")]
    y = Inches(2.0)
    for q, _a in lines:
    ...
    exercise_solucion(s, Inches(8.4), Inches(1.9), Inches(4.4), Inches(4.0),
        [[("1. está", {"color": GD, "size": 14})], [("2. Está", {"color": GD, "size": 14})],
         [("3. debajo", {"color": GD, "size": 14})], [("4. puedes", {"color": GD, "size": 14})]],
        title_doc="SOLUCIÓN · docent")
    noodroute(s)
    notes(s, "Laat leerlingen eerst zelf proberen (in duo). Klik daarna de oplossing open. Let op: ¿dónde está…? / está = plaats van een bekend ding; debajo de la mesa (preposición); ¿puedo…? (ik vraag) → antwoord «sí, puedes» (jij mag). de+el=del (encima del sofá). Oplossing: está · Está · debajo · puedes.")
    footer(s, tab=FTAB, page=pg())

def s09_speaking():
    s = slide(); bg(s)
    sectionbar(s, "§3 · HABLAR", "Describe una habitación", "Beschrijf een kamer: wat er is & waar het staat — sin leer", num=3)
    ...
    text(s, Inches(0.85), Inches(1.95), Inches(7.1), Inches(3.0),
         [[("Modelo · zeg dit hardop:", {"size": 13, "bold": True, "color": GD, "font": DISPLAY})],
          [("«— En el salón hay un sofá.", {"size": 18, "color": INK})],
          [("Está encima de una alfombra,", {"size": 18, "color": INK})],
          [("al lado de la ventana.»", {"size": 18, "color": INK})]])
    ...
    text(s, Inches(8.65), Inches(1.95), Inches(3.9), Inches(3.0),
         [[("¿Cómo? · Werkvorm", {"size": 13, "bold": True, "color": GD, "font": DISPLAY})],
          [("1. Kies een kamer (la cocina, el salón…).", {"size": 13, "color": INK})],
          [("2. ¿Qué hay? «En … hay un/una…».", {"size": 13, "color": INK})],
          [("3. ¿Dónde está? «Está + preposición».", {"size": 13, "color": INK})],
...
def s10_musica():
    s = slide(); bg(s)
    sectionbar(s, "CULTURA", "Casas del mundo hispano", "Typische huizen & plekken + muziek die jullie kennen", num=None)
    ...
    text(s, Inches(0.85), Inches(5.65), Inches(11.7), Inches(0.8),
         [[("🏠 Casas y lugares típicos ", {"size": 13, "bold": True, "color": GD, "font": DISPLAY}),
           ("(el patio andaluz · la casa colonial de Cartagena · el balcón · la hamaca del Caribe). La playlist + LyricsTraining op de digitale hub → tabblad Música.", {"size": 12, "color": INK})]])
    footer(s, tab=FTAB, page=pg())

def s11_tarea():
    s = slide(); bg(s)
    sectionbar(s, "§5 · TAREA FINAL", "Plano de mi casa", "Teken je plattegrond, benoem de kamers & zeg waar alles staat", num=5)
    ...
    text(s, Inches(0.75), Inches(1.78), Inches(5.6), Inches(0.4), [[("PLANO · noteer je frames", {"size": 12, "bold": True, "color": WHITE, "font": DISPLAY})]])
    text(s, Inches(0.85), Inches(2.5), Inches(5.4), Inches(2.5),
         [[("🏠 Aquí está ", {"size": 14, "bold": True, "color": INK}), ("____ (la cocina · el salón…).", {"size": 14, "color": LINE})],
          [("📦 En … hay ", {"size": 14, "bold": True, "color": INK}), ("____ (un sofá · una cama…).", {"size": 14, "color": LINE})],
          [("📍 Está ", {"size": 14, "bold": True, "color": INK}), ("____ (encima de · al lado del…).", {"size": 14, "color": LINE})]])
    ...
    text(s, Inches(7.05), Inches(1.9), Inches(5.5), Inches(3.1),
         [[("Los pasos · stappen", {"size": 14, "bold": True, "color": GD, "font": DISPLAY})],
          [("1. Dibuja el plano (4 habitaciones).", {"size": 13, "color": INK})],
          [("2. Etiqueta: «Aquí está la cocina…».", {"size": 13, "color": INK})],
          [("3. Sitúa: «hay un sofá · está encima de…».", {"size": 13, "color": INK})],
    ...
    text(s, Inches(0.6), Inches(5.5), Inches(12), Inches(0.9),
         [[("🏁 Klaar als… ", {"size": 13, "bold": True, "color": GD, "font": DISPLAY}),
           ("je 4 kamers benoemt en per kamer zegt wat er is («hay…») én waar het staat (preposición + del) — zónder af te lezen.", {"size": 12.5, "color": INK})]])
    footer(s, tab=FTAB, page=pg())
...
def s12_repaso():
    ...
    text(s, Inches(0.85), Inches(1.9), Inches(7.1), Inches(4.0),
         [[("Zo zeg je waar iets is", {"size": 14, "bold": True, "color": GD, "font": DISPLAY})],
          [("¿Dónde está…? — Está encima de / debajo de / al lado del…", {"size": 13.5, "color": INK})],
          [("encima · debajo · dentro · al lado · delante · detrás · entre", {"size": 13, "color": INK})],
          [("de + el = del · a + el = al (al lado del armario)", {"size": 13, "color": INK})],
          [("", {"size": 6})],
          [("hay vs. está · pedir permiso", {"size": 14, "bold": True, "color": GD, "font": DISPLAY})],
          [("hay = iets nieuw (er is…) · está = waar dat bekende ding staat", {"size": 13, "color": INK})],
          [("¿Puedo…? (ik) · ¿Puedes…? (jij) + hele werkwoord", {"size": 13, "color": INK})],
          [("la cocina · el salón · el dormitorio · el cuarto de baño", {"size": 13, "color": INK})]])
    ...
    items = ["de kamers benoemen (la cocina, el salón…)", "preposiciones de lugar gebruiken", "de+el=del / a+el=al juist maken", "hay / está juist kiezen", "toestemming vragen (¿puedo? / ¿puedes?)"]
    y = Inches(2.5)
    for it in items:
        text(s, Inches(8.65), y, Inches(3.9), Inches(0.7), [[("🟢🟡🔴  ", {"size": 12}), (it, {"size": 11.5, "color": INK})]])
        y = y + Inches(0.62)
    text(s, Inches(8.65), Inches(5.6), Inches(3.9), Inches(0.7), [[("🎮 Repasa jugando", {"size": 12, "bold": True, "color": GD, "font": DISPLAY})], [("online op de hub · plano de mi casa", {"size": 11, "italic": True, "color": MUT})]])
    footer(s, tab=FTAB, page=pg())

def s13_teacher():
    s = slide(); bg(s, color=RGBColor(0x24,0x1C,0x1B))
    text(s, Inches(0.6), Inches(0.5), Inches(12), Inches(0.7), [[("Docentendossier · Unidad 6 «La casa y los lugares»", {"size": 22, "bold": True, "color": WHITE, "font": DISPLAY})]])
    text(s, Inches(0.6), Inches(1.4), Inches(12.1), Inches(5.4),
         [[("Timing (2 lesuren van 50 min).", {"size": 14, "bold": True, "color": RGBColor(0xFB,0xEA,0xEC), "font": DISPLAY})],
          [("Les 1: Escucha (sitcom ep. 6 · id oDu5s6q4YKg, Julio is nerveus: María's spullen liggen overal) + Suena bien (b = v · llana ↔ aguda) + Kit (las habitaciones · muebles · preposiciones de lugar). Les 2: gramática functioneel (preposiciones de lugar + de+el=del/a+el=al · hay vs. está · poder ¿puedo?/¿puedes?), hablar «describe una habitación», tarea «Plano de mi casa» + cultura.", {"size": 12.5, "color": RGBColor(0xEC,0xEA,0xE3)})],
          [("", {"size": 6})],
          [("Aanpak C4 (survival).", {"size": 14, "bold": True, "color": RGBColor(0xFB,0xEA,0xEC), "font": DISPLAY})],
          [("Chunks komen auditief binnen (luisteren → naspreken). Preposiciones de lugar, hay/está en poder enkel functioneel (vaste vormen, herkennen & gebruiken), géén volledig werkwoordsysteem — dat is C5. b=v = één klank (betacismo): baca/vaca klinken gelijk, enkel de spelling verschilt. Kernvalstrik: de+el=del (encima del sofá, nooit «de el») en a+el=al; hay = iets nieuw ↔ está = waar dat bekende ding staat; poder + hele werkwoord (fumar, niet fumo). Doelcodes: C4-TS-3 · C4-WS-1 · C4-GE-2 · C4-SP-2 · C4-MEC-1/2 · C4-CU-1.", {"size": 12.5, "color": RGBColor(0xEC,0xEA,0xE3)})],
          [("", {"size": 6})],
          [("Evaluatie.", {"size": 14, "bold": True, "color": RGBColor(0xFB,0xEA,0xEC), "font": DISPLAY})],
          [("Mondelinge mini-taak («Plano de mi casa»: 4 kamers benoemen, per kamer zeggen wat er is met «hay» en waar het staat met een preposición + del). Geen leerplan → focus op «kunnen gebruiken in de praktijk». Rubric: habitaciones benoemen · hay/está + preposición correct · uitspraak & durf.", {"size": 12.5, "color": RGBColor(0xEC,0xEA,0xE3)})],
          [("", {"size": 6})],
...
def s_uitspraak():
    s = slide(); bg(s)
    sectionbar(s, "SUENA BIEN", "La b = la v · llana ↔ aguda", "b en v klinken hetzelfde & waar ligt de klemtoon — luister en spreek na", num=None)
    # b=v-kaart
    ...
    text(s,Inches(0.8),Inches(1.85),Inches(5.6),Inches(0.4),[[("La b y la v · un solo sonido",{"size":14,"bold":True,"color":GD,"font":DISPLAY})]])
    text(s,Inches(0.8),Inches(2.35),Inches(5.6),Inches(1.3),
         [[("bien · vino · bueno · vivir · beber · ventana",{"size":15,"bold":True,"color":INK})],
          [("b en v klinken nét hetzelfde (betacismo) — één klank.",{"size":11,"italic":True,"color":MUT})]])
    # llana/aguda-kaart
    ...
    text(s,Inches(7.0),Inches(1.85),Inches(5.6),Inches(0.4),[[("Llana ↔ aguda · la fuerza",{"size":14,"bold":True,"color":GD,"font":DISPLAY})]])
    text(s,Inches(7.0),Inches(2.35),Inches(5.6),Inches(1.3),
         [[("ca·SA · co·CI·na (llana)  ↔  sa·LÓN · so·FÁ (aguda)",{"size":14,"bold":True,"color":INK})],
          [("llana = klemtoon voorlaatste · aguda = laatste lettergreep.",{"size":11,"italic":True,"color":MUT})]])
    ...
    text(s,Inches(0.85),Inches(4.05),Inches(11.7),Inches(0.6),
         [[("¡Ojo! ",{"size":13,"bold":True,"color":RED,"font":DISPLAY}),("ba",{"size":13,"color":INK}),("c",{"size":13,"bold":True,"color":GD}),("a (imperiaal) klinkt als ",{"size":13,"color":INK}),("v",{"size":13,"bold":True,"color":GD}),("aca (koe) · ",{"size":13,"color":INK}),("b",{"size":13,"bold":True,"color":GD}),("ien = ",{"size":13,"color":INK}),("v",{"size":13,"bold":True,"color":GD}),("ien-klank. De klank is gelijk — enkel de spelling verschilt!",{"size":13,"color":INK})]])
    text(s,Inches(0.6),Inches(5.0),Inches(12),Inches(0.4),[[("La tilde · waar ligt de klemtoon? — het accent staat op de sterkste lettergreep",{"size":14,"bold":True,"color":GD,"font":DISPLAY})]])
    text(s,Inches(0.6),Inches(5.55),Inches(12.2),Inches(0.7),
         [[("ca·",{}),("SA",{"color":G,"bold":True}),("      so·",{}),("FÁ",{"color":G,"bold":True}),("      sa·",{}),("LÓN",{"color":G,"bold":True}),("      co·",{}),("CI",{"color":G,"bold":True}),("·na",{})]],size=20,font=DISPLAY)
...
def s_funciones():
    ...
    # adaptieve rijhoogte: bij 14 funciones (U6, incl. de nieuwe F13/F14) mag geen
