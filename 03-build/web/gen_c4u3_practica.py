#!/usr/bin/env python3
# C4 · Unidad 3 — §3 PRÁCTICA (native, zelfcorrigerend). Oefeningen op de chunks/woordenschat
# uit de scène (¿de dónde eres? · soy de + país · gentilicio m/v · idiomas — functioneel).
# Receptief → gestuurd → productief. C4-rood, TTS, motor-vrij standalone. Zelfde pijplijn als U1/U2.
import base64, os
...
<meta name="viewport" content="width=device-width,initial-scale=1"><title>C4 · U3 · Práctica</title><style>__CSS__</style></head><body>
<div class="top"><h1>Unidad 3 · Nacionalidades y países — Práctica</h1><p>Practica la lengua útil de la escena: de <b>reconocer</b> → <b>elegir</b> → <b>decirlo y escribirlo tú</b>. Todo se corrige solo; pulsa 🔊 para oírlo. <span class="stn">van herkennen naar zelf zeggen</span></p></div>
<main>
...
// data — U3: ¿de dónde eres? · soy de + país · gentilicio m/v · idiomas
var CHUNKS=[["¿De dónde eres?","waar kom je vandaan?"],["Soy de Bélgica","ik kom uit België"],["Soy de España","ik kom uit Spanje"],["Soy de México","ik kom uit Mexico"],["español/a","Spaans"],["belga","Belgisch"],["mexicano/a","Mexicaans"],["colombiano/a","Colombiaans"],["argentino/a","Argentijns"],["Hablo español","ik spreek Spaans"],["Hablo neerlandés","ik spreek Nederlands"],["¿Qué idiomas hablas?","welke talen spreek je?"],["un poco","een beetje"],["árabe","Arabisch"],["francés","Frans"],["inglés","Engels"],["Perú","Peru"],["Argentina","Argentinië"]];
...
// ① Clasifica (país / gentilicio / idioma)
(function(){var el=document.getElementById('g_clasifica');
 var items=[["España","país"],["México","país"],["Colombia","país"],["mexicana","gentilicio"],["argentino","gentilicio"],["belga","gentilicio"],["el neerlandés","idioma"],["el árabe","idioma"],["el inglés","idioma"]];
 var cat={"país":"país 🌍","gentilicio":"gentilicio 🪪","idioma":"idioma 🗣️"};var pool=items.slice(),p=0,s=0;
 el.innerHTML='<h3>Clasifica 🎯</h3><p class="desc">¿Es un país, una nacionalidad o un idioma? Elige. <span class="stn">land, nationaliteit of taal?</span></p>'+sb('sbC')+'<div id="cw" style="font-size:24px;font-family:var(--disp);text-align:center;margin:8px 0"></div><div class="chips" id="cc" style="justify-content:center"></div><div class="fb" id="cf"></div>';
...
// ② Match país↔gentilicio
(function(){var el=document.getElementById('g_match');
 var pairs=[["España","español / española"],["México","mexicano / mexicana"],["Colombia","colombiano / colombiana"],["Argentina","argentino / argentina"],["Bélgica","belga"],["Francia","francés / francesa"]];
 var p=0,s=0,sel=null,done=0;var L=pairs.map(x=>x[0]),R=pairs.map(x=>x[1]).slice().sort(()=>Math.random()-.5);
 el.innerHTML='<h3>Relaciona país ↔ nacionalidad 🧩</h3><p class="desc">Klik links een land, dan rechts de juiste nationaliteit (♂/♀).</p>'+sb('sbM')+'<div style="display:grid;grid-template-columns:1fr 1fr;gap:14px"><div class="chips" style="flex-direction:column" id="mL"></div><div class="chips" style="flex-direction:column" id="mR"></div></div><div class="fb" id="mf"></div>';
...
// ② Completa el diálogo (¿de dónde eres?)
(function(){var el=document.getElementById('g_completa');
 var gaps=[["eres"],["Soy"],["mexicano"],["hablas"],["Hablo"]];
 el.innerHTML='<h3>Completa el diálogo 💬</h3><p class="desc">Vul de ontbrekende woorden in en klik Controleer.</p>'
  +'<div class="dlgfill">— Hola, ¿de dónde <input class="txin" data-i="0">? <br>— <input class="txin" data-i="1"> de México. Soy <input class="txin" data-i="2"> <span style="color:var(--mut);font-size:12px">(un chico)</span>.<br>— ¿Qué idiomas <input class="txin" data-i="3">?<br>— <input class="txin" data-i="4"> español e inglés.</div>'
...
// ② Ordena la conversación (origen)
(function(){var el=document.getElementById('g_ordena');
 var sol=["Hola, ¿de dónde eres?","Soy de Colombia, de Cartagena.","¡Qué bien! ¿Y qué idiomas hablas?","Hablo español y un poco de inglés.","Yo soy de Bélgica y hablo neerlandés.","¡Encantado! Hasta luego."];
 ...
 el.querySelector('#oC').onclick=function(){fb(el.querySelector('#oF'),cur.join('|')===sol.join('|'),cur.join('|')===sol.join('|')?'¡Perfecto!':'Todavía no — empieza preguntando por el país.');};
...
// ② gentilicio (-o/-a) — m/v inductief
(function(){var el=document.getElementById('g_oa');
 var items=[["Diego (chico) es de México:","mexican","o"],["Valen (chica) es de Colombia:","colombian","a"],["Mateo (chico) es de Argentina:","argentin","o"],["Nina (chica) es de Perú:","peruan","a"],["Un chico de Chile:","chilen","o"]];
 var pool=items.slice(),p=0,s=0;
 el.innerHTML='<h3>La nacionalidad… ¿-o o -a? 👀</h3><p class="desc">♂ zegt -o, ♀ zegt -a. Kies de juiste letter.</p>'+sb('sbO')+'<div id="ow" style="font-size:18px;text-align:center;margin:8px 0"></div><div class="chips" style="justify-content:center"><div class="chip" onclick="window._oa(\'o\')">-o</div><div class="chip" onclick="window._oa(\'a\')">-a</div></div><div class="fb" id="of"></div>';
 var bar=el.querySelector('#sbO');function nx(){if(!pool.length)pool=items.slice();el.cur=pool.splice(Math.floor(Math.random()*pool.length),1)[0];el.querySelector('#ow').innerHTML=el.cur[0]+' es <b>'+el.cur[1]+'__</b>';el.querySelector('#of').className='fb';}
 window._oa=function(k){var ok=k===el.cur[2];if(ok){p++;s++}else s=0;setSc(bar,p,s);fb(el.querySelector('#of'),ok,'es '+el.cur[1]+el.cur[2]);setTimeout(nx,950);};nx();})();

// ② ¿Tú o usted?
(function(){var el=document.getElementById('g_tuusted');
 var items=[["¿De dónde eres?","tú"],["¿De dónde es usted?","usted"],["¿Y tú?","tú"],["¿Habla usted francés?","usted"],["¿Qué idiomas hablas?","tú"]];
 var pool=items.slice(),p=0,s=0;
 ...
 window._tu=function(k){var ok=k===el.cur[1];if(ok){p++;s++}else s=0;setSc(bar,p,s);fb(el.querySelector('#tf'),ok,el.cur[1]==='tú'?'informal (con amigos y compañeros)':'beleefd (met een onbekende/volwassene)');setTimeout(nx,1100);};nx();})();

// ③ ¿De dónde eres? (vrije productie)
(function(){var el=document.getElementById('g_presentate');
 el.innerHTML='<h3>¿De dónde eres? ✍️🗣️</h3><p class="desc">Di de dónde eres, tu nacionalidad y qué idiomas hablas. No hay respuesta «correcta»: es tu presentación. <span class="stn">het is jouw voorstelling</span></p>'
  +'<div style="display:grid;gap:8px;max-width:560px"><div><span style="font-family:var(--disp)">Yo soy de</span> <input class="txin" id="p1" placeholder="Bélgica"> <span style="font-family:var(--disp)">. Soy</span> <input class="txin" id="p2" placeholder="belga"> <span style="font-family:var(--disp)">.</span></div>'
  +'<div><span style="font-family:var(--disp)">Hablo</span> <input class="txin" id="p3" placeholder="neerlandés y un poco de español"> <span style="font-family:var(--disp)">.</span></div></div>'
 ...
 el.querySelector('#pGo').onclick=function(){var a=(el.querySelector('#p1').value||'Bélgica').trim(),b=(el.querySelector('#p2').value||'belga').trim(),c=(el.querySelector('#p3').value||'neerlandés').trim();el.cur='¡Hola! Yo soy de '+a+'. Soy '+b+' y hablo '+c+'.';fb(el.querySelector('#pF'),true,'<b>'+el.cur+'</b><br><span style="color:var(--mut);font-style:italic">Zeg het nu hardop tegen je buur, en wijs je land aan op de kaart.</span>');};
...
open(f"{ROOT}/03-build/web/componentes/C4_U3_practica.html","w").write(html)
print("C4_U3_practica.html geschreven:",len(html),"bytes")
