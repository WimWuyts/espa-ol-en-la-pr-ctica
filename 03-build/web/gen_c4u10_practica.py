...
<div class="top"><h1>Unidad 10 · Las tareas de casa — Práctica</h1><p>Practica la lengua útil de la escena: de <b>reconocer</b> → <b>elegir</b> → <b>decirlo y escribirlo tú</b>. Todo se corrige solo; pulsa 🔊 para oírlo. <span class="stn">van herkennen naar zelf zeggen</span></p></div>
<main>
...
(function(){var el=document.getElementById('g_clasifica');
 ...
 el.innerHTML='<h3>Clasifica 🎯</h3><p class="desc">¿Es una <b>tarea de casa</b>, <b>ofrecer ayuda</b> o decir lo que <b>sabes hacer</b>? <span class="stn">huistaak, hulp of kunnen?</span></p>'+sb('sbC')+'<div class="big" id="cw"></div><div class="chips" id="cc" style="justify-content:center"></div><div class="fb" id="cf"></div>';
...
(function(){var el=document.getElementById('g_hayque');
 ...
 el.innerHTML='<h3>¿hay que · tengo que · tienes que? ✅</h3><p class="desc"><b>hay que</b> = tiene que hacerse (general) · <b>tengo que</b> = yo debo · <b>tienes que</b> = tú debes. <span class="stn">algemeen · ík · jíj</span></p>'+sb('sbH')+'<div class="big" id="hw"></div><div class="chips" style="justify-content:center"><div class="chip" onclick="window._hq(\'hay que\')">hay que (algemeen)</div><div class="chip" onclick="window._hq(\'tengo que\')">tengo que (ik)</div><div class="chip" onclick="window._hq(\'tienes que\')">tienes que (jij)</div></div><div class="fb" id="hf"></div>';
...
(function(){var el=document.getElementById('g_match');
 ...
 el.innerHTML='<h3>Relaciona · tarea ↔ objeto 🧩</h3><p class="desc">Pulsa a la izquierda una tarea y a la derecha lo que necesitas para hacerla (o dónde la haces). <span class="stn">verbind taak en middel</span></p>'+sb('sbM')+'<div class="grid2"><div class="chips" style="flex-direction:column" id="mL"></div><div class="chips" style="flex-direction:column" id="mR"></div></div><div class="fb" id="mf"></div>';
...
(function(){var el=document.getElementById('g_ordena');
 ...
 el.querySelector('#oC').onclick=function(){var ok=cur.join('|')===sol.join('|');fb(el.querySelector('#oF'),ok,ok?'¡Perfecto!':'Todavía no — empieza por el problema (la asistenta).');};
...
(function(){var el=document.getElementById('g_reparto');
 ...
 el.innerHTML='<h3>¿Quién hace qué en mi casa? ✍️🗣️</h3><p class="desc">Elige tres tareas y di quién las hace. No hay respuesta «correcta»: es tu casa. <span class="stn">het is jouw huis</span></p>'
 ...
 el.querySelector('#rGo').onclick=function(){
  ...
  fb(el.querySelector('#rF'),true,'<b>'+el.cur+'</b><br><span style="color:var(--mut);font-style:italic">Dilo en voz alta. Después pregúntale a tu compañero/a: «¿Y en tu casa? ¿Quién friega los platos?», y ofrece ayuda con «Yo te ayudo».</span>');};
