...
.nl{display:block;color:var(--mut);font-style:italic}
...
<div class="top"><h1>Unidad 1 · Presentaciones — Práctica</h1><p>Practica el español de la escena: de <b>reconocer</b> → <b>elegir</b> → <b>decirlo y escribirlo tú</b>. Todo se corrige solo; pulsa 🔊 para escuchar. <span class="nl">Oefen de taal uit de scène: herkennen → kiezen → zelf zeggen en schrijven. Alles corrigeert zichzelf; klik 🔊 om te horen.</span></p></div>
<main>
 <h2 class="subh">① Reconocer <span class="pill">herkennen</span></h2>
 ...
 <h2 class="subh">③ Producir y comunicar <span class="pill">zelf zeggen</span></h2>
...
// ① Tarjetas
(function(){var el=document.getElementById('g_cards');el.innerHTML='<h3>Tarjetas 🗂️</h3><p class="desc">Gira la tarjeta (ES↔NL) y pulsa 🔊 para escuchar. <span class="nl">Draai de kaart; klik 🔊 om te horen.</span></p><div class="fcgrid" id="cg"></div>';
...
// ① Escucha
(function(){var el=document.getElementById('g_escucha');if(!TTS){el.innerHTML='<h3>¿Qué oyes? 🎧</h3><p class="desc">El audio funciona en Chrome/Edge. <span class="nl">Spraak werkt in Chrome/Edge.</span></p>';return;}
 var bank=CHUNKS.map(c=>c[0].replace('…','')).filter(x=>x.length<16);var p=0,s=0;
 el.innerHTML='<h3>¿Qué oyes? 🎧</h3><p class="desc">Pulsa ▶, escucha y elige lo que oyes. <span class="nl">Pulsa ▶, escucha y elige lo que oyes. <span class="stn">luister en kies</span></span></p>'+sb('sbE')+'<div style="margin:6px 0"><button class="spk-btn" id="eP">▶ Escuchar</button></div><div class="chips" id="eO"></div><div class="fb" id="eF"></div>';
...
(function(){var el=document.getElementById('g_clasifica');
 ...
 el.innerHTML='<h3>Clasifica 🎯</h3><p class="desc">¿Qué es? Elige la categoría. <span class="nl">Wat is het? Kies de categorie.</span></p>'+sb('sbC')+'<div id="cw" style="font-size:24px;font-family:var(--disp);text-align:center;margin:8px 0"></div><div class="chips" id="cc" style="justify-content:center"></div><div class="fb" id="cf"></div>';
...
(function(){var el=document.getElementById('g_match');
 ...
 el.innerHTML='<h3>Relaciona 🧩</h3><p class="desc">Pulsa una expresión a la izquierda y su situación a la derecha. <span class="nl">Klik links een uitdrukking, dan rechts de situatie.</span></p>'+sb('sbM')+'<div style="display:grid;grid-template-columns:1fr 1fr;gap:14px"><div class="chips" style="flex-direction:column" id="mL"></div><div class="chips" style="flex-direction:column" id="mR"></div></div><div class="fb" id="mf"></div>';
...
(function(){var el=document.getElementById('g_completa');
 ...
 el.innerHTML='<h3>Completa el diálogo 💬</h3><p class="desc">Escribe las palabras que faltan y pulsa «Comprobar». <span class="nl">Vul de ontbrekende woorden in en klik op Controleer.</span></p>'
  ...
  +'<div class="answerbtns"><button class="btn" id="coGo">Comprobar</button></div><div class="fb" id="coF"></div>';
...
(function(){var el=document.getElementById('g_ordena');
 ...
 el.innerHTML='<h3>Ordena la conversación 🔢</h3><p class="desc">Pulsa las frases en el orden correcto. <span class="nl">Klik de zinnen in de juiste volgorde.</span></p><div class="chips" id="oP"></div><div class="col" id="oB"><b style="font-size:12px;color:var(--gd)">TU ORDEN</b><div class="chips" id="oBB" style="margin-top:6px"></div></div><div class="answerbtns"><button class="btn sec" id="oR">Reset</button><button class="btn" id="oC">Comprobar</button></div><div class="fb" id="oF"></div>';
 ...
 el.querySelector('#oC').onclick=function(){fb(el.querySelector('#oF'),cur.join('|')===sol.join('|'),cur.join('|')===sol.join('|')?'¡Perfecto!':'Todavía no — empieza por un saludo.');};
...
(function(){var el=document.getElementById('g_oa');
 ...
 el.innerHTML='<h3>¿-o o -a? 👀</h3><p class="desc">Un chico dice -o, una chica dice -a. Elige la letra correcta. <span class="nl">Jongens zeggen -o, meisjes -a. Kies de juiste letter.</span></p>'+sb('sbO')+'<div id="ow" style="font-size:18px;text-align:center;margin:8px 0"></div><div class="chips" style="justify-content:center"><div class="chip" onclick="window._oa(\'o\')">-o</div><div class="chip" onclick="window._oa(\'a\')">-a</div></div><div class="fb" id="of"></div>';
...
(function(){var el=document.getElementById('g_tuusted');
 ...
 var pool=items.slice(),p=0,s=0;
 el.innerHTML='<h3>¿Tú o usted? 🤝</h3><p class="desc">¿Informal (tú) o formal (usted)? Elige. <span class="nl">Informeel of beleefd? Kies.</span></p>'+sb('sbTU')+'<div id="tw" style="font-size:19px;font-family:var(--disp);text-align:center;margin:8px 0"></div><div class="chips" style="justify-content:center"><div class="chip" onclick="window._tu(\'tú\')">tú (informeel)</div><div class="chip" onclick="window._tu(\'usted\')">usted (beleefd)</div></div><div class="fb" id="tf"></div>';
...
(function(){var el=document.getElementById('g_presentate');
 el.innerHTML='<h3>Preséntate ✍️🗣️</h3><p class="desc">Preséntate: completa, forma tu frase y dila en voz alta. No hay una respuesta «correcta»: es tu presentación. <span class="nl">Stel jezelf voor: vul in, maak je zin en zeg ze hardop. Er is geen «juist» — het is jouw voorstelling.</span></p>'
