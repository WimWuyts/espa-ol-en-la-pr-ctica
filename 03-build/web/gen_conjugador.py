...
function renderChips(){const q=(document.getElementById('vsearch').value||'').toLowerCase();const f=document.getElementById('vfilter').value;
  const box=document.getElementById('vchips');box.innerHTML='';let n=0;
  DB.forEach(v=>{if(q&&!(v.inf.includes(q)||v.nl.toLowerCase().includes(q)))return;if(!matchType(v,f))return;n++;
    const c=document.createElement('div');c.className='vchip'+(curV&&v.inf===curV.inf?' on':'');c.innerHTML=v.inf+' <small>'+v.nl+'</small>';c.onclick=()=>{curV=v;renderChips();renderConj();};box.appendChild(c);});
  document.getElementById('vcount').textContent=n+' werkwoorden';}
