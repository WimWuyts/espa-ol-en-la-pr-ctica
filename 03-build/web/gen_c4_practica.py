...
(function(){var m=marco('g_cards','① Tarjetas · ¿qué significa?',
 'Di el significado en voz alta antes de girar la tarjeta. Pulsa para girar, pulsa 🔊 para oírla. <span class=\"nl\">zeg het hardop vóór je omdraait</span>',D.pares.length);
...
(function(){var items=baraja(D.pares).slice(0,8);
 var m=marco('g_escucha','② ¿Qué oyes? · luister en kies',
  'Pulsa ▶, escucha y elige lo que oyes. ¿Dudas? Escucha otra vez. <span class="stn">luister en kies; twijfel je, luister nog eens</span>',items.length);
...
(function(){var m=marco('g_clasifica','③ Clasifica · ¿a qué grupo pertenece?',
 'Arrastra cada palabra a su grupo. ¿Mal? Vuelve a su sitio. <span class=\"nl\">sleep naar het juiste veld</span>',
...
(function(){var items=baraja(D.pares).slice(0,8);
 var m=marco('g_match','④ Empareja · español → nederlands',
  'Elige la traducción correcta. Ahora sin tarjetas: de memoria. <span class="stn">nu zonder kaartjes, uit het hoofd</span>',items.length);
 items.forEach(function(it){
...
(function(){var m=marco('g_completa','⑤ Completa · rellena el hueco',
 'Escribe el chunk que falta. No hace falta poner las tildes. <span class="stn">accenten mogen wegblijven</span>',D.huecos.length);
...
(function(){var m=marco('g_ordena','⑥ Ordena · pon las palabras en orden',
 'Pulsa las palabras en el orden correcto. Pulsa otra vez para quitar una. <span class=\"nl\">klik in de juiste volgorde</span>',D.ordena.length);
 D.ordena.forEach(function(o){
  ...
  q.querySelector('.btn').onclick=function(){
   ...
   fb.textContent=bien?'¡Correcto!':'Todavía no — escucha: '+o.frase;
...
(function(){var h=document.getElementById('g_grabate');
 ...
 h.innerHTML='<h3>⑦ Grábate · '+D.tarea.titulo+'</h3>'+
  '<p class="d">Haz la tarea final en voz alta y grábate. Escúchate: ¿te entiendes tú? <span class="stn">neem jezelf op en luister terug</span> '+
  'Después grábate otra vez: la segunda siempre sale mejor.</p><ol>'+pasos+'</ol>'+
  ...
  '<p class="d" id="rmsg">La grabación se queda en tu aparato: no se envía nada. <span class="stn">er wordt niets verstuurd</span></p>';
 ...
 go.onclick=function(){
  if(!navigator.mediaDevices||!window.MediaRecorder){
   msg.textContent='Este aparato no puede grabar en el navegador. Graba con el móvil y escúchate así.';return;}
  ...
  .catch(function(){msg.textContent='Sin acceso al micrófono. Actívalo en el navegador.';});};
...
<p>De <b>reconocer</b> a <b>decirlo tú</b>. Todo se corrige solo; pulsa 🔊 para oírlo. <span class="stn">van herkennen naar zelf zeggen</span></p></div>
<main>
 <h2 class="subh">① Reconocer <span class="pill">lo ves y lo reconoces</span></h2>
 <div class="game" id="g_cards"></div>
 <div class="game" id="g_escucha"></div>
 <div class="game" id="g_clasifica"></div>
 <h2 class="subh">② Practicar <span class="pill">con apoyo</span></h2>
 <div class="game" id="g_match"></div>
 <div class="game" id="g_completa"></div>
 <div class="game" id="g_ordena"></div>
 <h2 class="subh">③ Producir <span class="pill">sin apoyo</span></h2>
