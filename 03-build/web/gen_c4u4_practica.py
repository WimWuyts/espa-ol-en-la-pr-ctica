#!/usr/bin/env python3
# C4 · Unidad 4 — §3 PRÁCTICA (native, zelfcorrigerend). La familia · presentar & describir.
import base64, os
...
var CHUNKS=[["la madre","de moeder"],["el padre","de vader"],["el hermano","de broer"],["la hermana","de zus"],["el abuelo","de opa"],["la abuela","de oma"],["el tío","de oom"],["la tía","de tante"],["es alto/a","is lang"],["es guapo/a","is knap"],["es simpático/a","is aardig"],["es divertido/a","is grappig"],["es inteligente","is intelligent"],["es amable","is vriendelijk"],["Esta es mi madre","dit is mijn moeder"],["muy","heel"],["un poco","een beetje"]];
...
// ① Clasifica (familia / físico / carácter)
...
// ② Match miembro ↔ definición
(function(){var el=document.getElementById('g_match');
 var pairs=[["la abuela","la madre de mi madre"],["el tío","el hermano de mi padre"],["el abuelo","el padre de mi padre"],["la hermana","la hija de mis padres (chica)"],["los padres","la madre y el padre"]];
 var p=0,s=0,sel=null,done=0;var L=pairs.map(x=>x[0]),R=pairs.map(x=>x[1]).slice().sort(()=>Math.random()-.5);
 el.innerHTML='<h3>Relaciona · ¿quién es? 🧩</h3><p class="desc">Klik links een familielid, dan rechts de juiste omschrijving.</p>'+sb('sbM')+'<div style="display:grid;grid-template-columns:1fr 1fr;gap:14px"><div class="chips" style="flex-direction:column" id="mL"></div><div class="chips" style="flex-direction:column" id="mR"></div></div><div class="fb" id="mf"></div>';
...
(function(){var el=document.getElementById('g_completa');
 var gaps=[["Es"],["simpática"],["poco"],["hermano"],["o"]];
 el.innerHTML='<h3>Completa el diálogo 💬</h3><p class="desc">Vul aan (denk aan ser + adjetivo en ♂/♀).</p>'
  +'<div class="dlgfill">— ¿Quién es esta chica? <br>— <input class="txin" data-i="0"> mi hermana. <br>— ¿Cómo es?<br>— Es muy <input class="txin" data-i="1"> <span style="color:var(--mut);font-size:12px">(♀ aardig)</span> y un <input class="txin" data-i="2"> tímida.<br>— ¿Y este chico?<br>— Es mi <input class="txin" data-i="3">. Es alt<input class="txin" data-i="4" style="width:44px"> <span style="color:var(--mut);font-size:12px">(♂)</span>.</div>'
...
(function(){var el=document.getElementById('g_ordena');
 var sol=["¿Quién es esta persona?","Es mi abuela.","¿Y cómo es?","Es muy elegante y amable.","¿Dónde vive?","Vive en Madrid."];
...
(function(){var el=document.getElementById('g_oa');
 var items=[["Mi tío (chico) es","guap","o"],["Mi hermana (chica) es","alt","a"],["María (chica) es","delgad","a"],["Mi abuelo (chico) es","simpátic","o"],["Una amiga (chica) es","divertid","a"]];
 var pool=items.slice(),p=0,s=0;
 el.innerHTML='<h3>El adjetivo… ¿-o o -a? 👀</h3><p class="desc">♂ zegt -o, ♀ zegt -a. Kies de juiste letter.</p>'+sb('sbO')+'<div id="ow" style="font-size:18px;text-align:center;margin:8px 0"></div><div class="chips" style="justify-content:center"><div class="chip" onclick="window._oa(\'o\')">-o</div><div class="chip" onclick="window._oa(\'a\')">-a</div></div><div class="fb" id="of"></div>';
...
(function(){var el=document.getElementById('g_presentate');
 ...
 el.querySelector('#pGo').onclick=function(){var r=el.querySelector('#p1').value,a=(el.querySelector('#p2').value||'Ana').trim(),c=(el.querySelector('#p3').value||'muy simpática').trim();var art=(r==='padre'||r==='hermano'||r==='tío')?'Este':'Esta';el.cur=art+' es mi '+r+'. Se llama '+a+'. Es '+c+'.';fb(el.querySelector('#pF'),true,'<b>'+el.cur+'</b><br><span style="color:var(--mut);font-style:italic">Zeg het nu hardop tegen je buur.</span>');};
...
open(f"{ROOT}/03-build/web/componentes/C4_U4_practica.html","w").write(html)
print("C4_U4_practica.html geschreven:",len(html),"bytes")
