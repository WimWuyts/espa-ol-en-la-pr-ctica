...
<div class="top"><h1>Unidad 8 · Las horas — Práctica</h1><p>Oefen de bruikbare taal uit de scène: van <b>herkennen</b> → <b>kiezen</b> → <b>zelf zeggen/schrijven</b>. Alles corrigeert zichzelf; klik 🔊 om te horen.</p></div>
<main>
...
(function(){var el=document.getElementById('g_clasifica');
 ...
 el.innerHTML='<h3>Clasifica 🎯</h3><p class="desc">Is het een uuraanduiding, een dag of een moment van de dag?</p>'+sb('sbC')+'<div id="cw" style="font-size:24px;font-family:var(--disp);text-align:center;margin:8px 0"></div><div class="chips" id="cc" style="justify-content:center"></div><div class="fb" id="cf"></div>';
...
(function(){var el=document.getElementById('g_dias');
 ...
 el.querySelector('#dC').onclick=function(){var ok=cur.join('|')===sol.join('|');fb(el.querySelector('#dF'),ok,ok?'¡Perfecto! lunes → domingo':'Nog niet — begin met lunes (maandag).');};
...
(function(){var el=document.getElementById('g_ordena');
 ...
 el.querySelector('#oC').onclick=function(){var ok=cur.join('|')===sol.join('|');fb(el.querySelector('#oF'),ok,ok?'¡Perfecto!':'Nog niet — begin met de vraag «¿Quieres quedar…?».');};
...
(function(){var el=document.getElementById('g_quedar');
 el.innerHTML='<h3>Queda conmigo ✍️🗣️</h3><p class="desc">Maak je eigen afspraak. Er is geen «juist» — het is jouw week.</p>'
